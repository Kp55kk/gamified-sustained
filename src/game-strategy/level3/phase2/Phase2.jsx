...
import{TreeScene3D,SolarScene3D,WindScene3D,Scene3DCanvas,playAction,playSuccess,playCorrect,playWrong}from'./Phase2Core';
import'./Phase2.css';

const PLANT_SPOTS=[[0,0,0],[3,0,2],[-3,0,1],[1.5,0,-2],[-2,0,-1.5]];
...
p+=0.02;setSunProgress(p);
if(p>=1){clearInterval(iv);setSunProgress(1);}
},100);
...
let idx=0;
const run=()=>{
const seq=WIND_SPEED_SEQUENCE[idx%WIND_SPEED_SEQUENCE.length];
setWindSpeed(seq.speed);setWindSeqIdx(idx);
idx++;
if(idx<=WIND_SPEED_SEQUENCE.length){
autoTimerRef.current=setTimeout(run,seq.duration);
}
};
run();
return()=>clearTimeout(autoTimerRef.current);
...
useEffect(()=>{
if(segId!=='trees'||segPhase!=='playing')return;
if(!task||task.id!=='grow')return;
let g=1;
...
if(last)n[n.length-1]={...last,growth:Math.min(4,g)};
return n;
});
if(g>=4){clearInterval(iv);setTreeGrowth(4);}
},800);
return()=>clearInterval(iv);
},[taskIdx,segPhase]);
...
if(task?.id==='absorb'){
setTrees(prev=>prev.map(t=>({...t,absorbing:true})));
}
},[taskIdx]);
...
const pos=PLANT_SPOTS[currentSpot]||[0,0,0];
...
if(task.id.startsWith('place_panel')){
setPanelsPlaced(p=>p+1);
showFB(task.feedback,'success');
advanceTask();
}else if(task.id==='orient'){
setShowOrient(true);
}
}else if(segId==='wind'){
if(task.id==='install_turbine'){
...
const o=PANEL_ORIENTATIONS[orientIdx];
setPanelAngle(o.angle);
setShowOrient(false);
if(o.correct){
playCorrect();
showFB('Panels aligned south — maximum sunlight!','success');
}else{
playWrong();
showFB(`${o.label} facing — only ${Math.round(o.efficiency*100)}% efficiency`,'warning');
}
advanceTask();
// Start sun simulation
setTimeout(()=>{setEnergyFlowing(true);},1500);
}
...
setPhase('comparison');setCompStep(0);
}else{
setSegIdx(nextSeg);setTaskIdx(0);setSegPhase('intro');
// Reset segment state
setPanelsPlaced(0);setPanelAngle(0);setOrientIdx(0);setSunProgress(0);setEnergyFlowing(false);
setTurbineInstalled(false);setWindSpeed(0);setCurrentSpot(p=>p+1);
}
}

function handlePlantMore(){
if(currentSpot+1<TOTAL_TREES_TO_PLANT){
setCurrentSpot(p=>p+1);setTaskIdx(0);setSegPhase('playing');
}else{
setSegPhase('data');setShowData(true);playSuccess();
}
}

function handleQuizSelect(idx){
if(quizSel!==null)return;
setQuizSel(idx);setShowExp(true);
if(idx===PHASE2_QUIZ[quizIdx].correctIndex){playCorrect();setQuizScore(p=>p+1);}
else playWrong();
}

function handleQuizNext(){
const next=quizIdx+1;
if(next>=PHASE2_QUIZ.length){
setStars(calculateP2Stars(segsComplete,SEGMENTS.length,quizScore,PHASE2_QUIZ.length));
setPhase('complete');playSuccess();
...
<div key={i} className={`l3p2-intro-line ${l.speaker==='teacher'?'teacher':''}`}
...
{SEGMENTS.map((s,i)=>(
<div key={s.id} className="l3p2-path-card" style={{'--c':s.color}}
onClick={()=>{setPhase('segments');setSegIdx(0);setSegPhase('intro');playAction();}}>
...
<div className="l3p2-container">
<Scene3DCanvas>
{segId==='trees'&&<TreeScene3D trees={trees} plantSpots={PLANT_SPOTS} currentSpot={currentSpot} co2Active={false} greenLevel={0}/>}
{segId==='solar'&&<SolarScene3D panelsPlaced={0} panelAngle={0} sunProgress={0.5} energyFlowing={false}/>}
{segId==='wind'&&<WindScene3D installed={false} windSpeed={2}/>}
</Scene3DCanvas>
...
<div className="l3p2-container">
<Scene3DCanvas>
{segId==='trees'&&<TreeScene3D trees={trees} plantSpots={PLANT_SPOTS} currentSpot={currentSpot}
co2Active={trees.some(t=>t.absorbing)} greenLevel={greenLevel}/>}
{segId==='solar'&&<SolarScene3D panelsPlaced={panelsPlaced} panelAngle={panelAngle}
sunProgress={sunProgress} energyFlowing={energyFlowing}/>}
{segId==='wind'&&<WindScene3D installed={turbineInstalled} windSpeed={windSpeed}/>}
</Scene3DCanvas>
...
<div className="l3p2-co2-bar">
<div className="l3p2-co2-fill" style={{width:`${co2}%`,background:co2Color}}/>
</div>
...
<div style={{position:'absolute',bottom:20,right:12,background:'#fff',border:'1px solid #e0e0e0',
borderRadius:16,padding:'14px 18px',zIndex:20,boxShadow:'0 4px 12px rgba(0,0,0,.08)',minWidth:160}}>
<div style={{fontSize:11,color:'#555',fontWeight:700,textTransform:'uppercase',letterSpacing:1}}>Wind Speed</div>
<div style={{fontFamily:"'Fredoka',sans-serif",fontSize:28,fontWeight:800,color:windSpeed>6?'#22c55e':windSpeed>3?'#f59e0b':'#ef4444'}}>
{windSpeed} m/s
</div>
<div style={{fontSize:12,color:'#888'}}>{WIND_SPEED_SEQUENCE[windSeqIdx%WIND_SPEED_SEQUENCE.length]?.label}</div>
<div style={{width:'100%',height:8,background:'#f0f0f0',borderRadius:4,marginTop:6,overflow:'hidden'}}>
<div style={{height:'100%',borderRadius:4,width:`${Math.min(100,windSpeed*8)}%`,
background:windSpeed>6?'#22c55e':windSpeed>3?'#f59e0b':'#ef4444',transition:'width 0.5s'}}/>
</div>
</div>
)}
...
<p style={{fontSize:13,color:'#555',marginBottom:12}}>Use ← → arrows or click to choose direction</p>
...
<div className="l3p2-orient-eff" style={{color:PANEL_ORIENTATIONS[orientIdx].efficiency>=0.9?'#22c55e':
PANEL_ORIENTATIONS[orientIdx].efficiency>=0.5?'#f59e0b':'#ef4444'}}>
...
{/* Feedback Toast */}
{feedback&&(
<div className={`l3p2-feedback ${feedback.type}`}>{feedback.text}</div>
)}
...
background:'rgba(34,197,94,0.2)',border:'2px solid rgba(34,197,94,0.5)',borderRadius:14,
padding:'12px 24px',color:'#86efac',fontWeight:700,fontSize:15,textAlign:'center'}}>
...
<div className="l3p2-comp-bar-outer">
<div className="l3p2-comp-bar-fill" style={{width:`${d.barWidth}%`,background:d.color}}>
{d.reductionPercent}%
</div>
</div>
...
<tr key={d.id}><td>{d.icon} {d.label}</td><td>{d.speed}</td><td>{d.co2PerYear}</td><td>{d.timeToEffect}</td></tr>
))}
</tbody>
</table>
...
<button className="l3p2-comp-continue" onClick={()=>{setPhase('realization');setRealStep(0);playAction();}}>
Continue →
</button>
</div>
</div>);
...
<div key={i} className={`l3p2-real-line ${l.speaker==='teacher'?'teacher':''}`}
style={{animationDelay:`${i*0.3}s`}}>
{l.speaker==='teacher'?'🧑‍🏫 ':''}{l.text}
</div>
))}
...
{showExp&&(<>
<div className="l3p2-quiz-exp">
{quizSel===q.correctIndex?'✅ Correct! ':'❌ Not quite. '}{q.explanation}
</div>
<button className="l3p2-quiz-next" onClick={handleQuizNext}>
{quizIdx+1>=PHASE2_QUIZ.length?'Finish Quiz →':'Next Question →'}
</button>
</>)}
</div>
</div>
...
{[1,2,3].map(s=>(
<span key={s} className={`l3p2-star ${s<=stars?'earned':''}`} style={{animationDelay:`${s*0.3}s`}}>⭐</span>
))}
</div>
...
<button className="l3p2-done-btn" onClick={()=>{setPhase('transition');playSuccess();}}>
Continue →
</button>
</div>
</div>
